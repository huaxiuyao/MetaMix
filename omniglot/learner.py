...
class Conv_Standard(nn.Module):
    ...
    def forward_metamix(self, hidden_support, label_support, hidden_query, label_query, weights, is_training=True):
        ...
        for layer in range(4):
            ...
            if not flag:
                hidden_support = self.functional_conv_block(hidden_support, weights[f'net.{layer}.0.weight'],
                                                            weights[f'net.{layer}.0.bias'],
                                                            weights.get(f'net.{layer}.1.weight'),
                                                            weights.get(f'net.{layer}.1.bias'),
                                                            is_training)
            hidden_query = self.functional_conv_block(hidden_query, weights[f'net.{layer}.0.weight'],
                                                      weights[f'net.{layer}.0.bias'],
                                                      weights.get(f'net.{layer}.1.weight'),
                                                      weights.get(f'net.{layer}.1.bias'),
                                                      is_training)

        hidden_query = torch.mean(hidden_query, dim=(2, 3))
    ...
    def functional_forward_cf(self, hidden, label, sel_layer, shuffle_list, shuffle_channel_id, weights,
        ...
        for layer in range(4):
            ...
            hidden = self.functional_conv_block(hidden, weights[f'net.{layer}.0.weight'],
                                                weights[f'net.{layer}.0.bias'],
                                                weights.get(f'net.{layer}.1.weight'),
                                                weights.get(f'net.{layer}.1.bias'),
                                                is_training)
