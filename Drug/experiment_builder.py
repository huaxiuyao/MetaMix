...
class ExperimentBuilder(object):
    def __init__(self, args, data, model, device, target_assay):
        """
        ...
        self.saved_models_filepath, self.logs_filepath, self.samples_filepath = build_experiment_folder(
            experiment_name=self.args.experiment_name.replace('assay',  f"assay{target_assay}"))
        ...
        self.data = data(args=args, current_iter=self.state['current_iter'], target_assay=self.target_assay)
    ...
    def train_iteration(self, train_sample, sample_idx, epoch_idx, total_losses, current_iter, pbar_train):
        ...
        target_set_x, target_set_y, target_set_z, target_set_assay, _ = train_sample

        data_batch = (support_set_x, support_set_y, support_set_z, support_set_assay,
                      target_set_x, target_set_y, target_set_z, target_set_assay)

        if sample_idx == 0:
            print("shape of data", support_set_x[0].shape, support_set_y[0].shape, support_set_z[0].shape, support_set_assay[0].shape,
                  target_set_x[0].shape, target_set_y[0].shape, target_set_z[0].shape, target_set_assay[0].shape)
    ...
    def evaluation_iteration(self, val_sample, total_losses, pbar_val, phase):
        ...
        """
        support_set_x, support_set_y, support_set_z, support_set_assay, \
        target_set_x, target_set_y, target_set_z, target_set_assay, _ = val_sample
    ...
    def test_evaluation_iteration(self, val_sample, model_idx, sample_idx, per_model_per_batch_preds, pbar_test):
        ...
        per_model_per_batch_preds[model_idx].extend(list(per_task_preds)[0])
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models):
        ...
        for idx, model_idx in enumerate(top_n_idx):
            ...
            with tqdm.tqdm(total=int(1)) as pbar_test:
                for sample_idx, test_sample in enumerate(
                        self.data.get_test_batches(total_batches=int(1))):
                    #print(test_sample[4])
                    per_model_per_batch_targets[idx].extend(np.array(test_sample[5][0]))
        ...
        #print(per_batch_max)
        # accuracy = np.mean(np.equal(per_batch_targets, per_batch_pred))
        accuracy = pearson_score(per_batch_targets, per_batch_preds)
        accuracy_std = 0.0
                    ...
                    if self.state['current_iter'] % self.args.total_iter_per_epoch == 0:
                        ...
                        with tqdm.tqdm(total=int(self.args.num_evaluation_tasks / self.args.batch_size)) as pbar_val:
                            for _, val_sample in enumerate(
                                    self.data.get_val_batches(total_batches=int(self.args.num_evaluation_tasks / self.args.num_evaluation_tasks))):
